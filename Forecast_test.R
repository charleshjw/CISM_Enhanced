...
IV_coef = matrix(NA, nrow = nrow(Model_Final), ncol = 2*(Max_var+4))
temporary_name = rbind(paste("Var",1:Max_var,"Coef",sep=""), paste("Var",1:Max_var,"SE",sep=""), paste("Var",1:Max_var,"p_val",sep="")) 
dim(temporary_name) <- NULL
colnames(IV_coef) = c("Intercept", "Interceptpval", temporary_name, "Var1CONF 2.5%", "Var1CONF 97.5%","Var2CONF 2.5%", "Var2CONF 97.5%")
...
#	i <- 2
	...
	coeff_pval = t(result$coefficients[,c(1,2,4)])
	conf = t(confint(temp_reg, level = 0.95))
	...
	IV_coef[i,1] = 0
	IV_coef[i,3:ncol(IV_coef)] = c(coeff_pval, rep(NA,5*Max_var-(length(coeff_pval)+ 4)), conf,  rep(NA,5*Max_var-(6+ length(conf))))
...
#rename column names
colnames(IV_coef) <- c("Intercept","Interceptpval","IV1_COEFF","IV1_SE","IV1_PVAL","IV2_COEFF","IV2_SE","IV2_PVAL","COEFF1_2.5%_Conf", "COEFF1_97.5%_Conf",  "COEFF2_2.5%_Conf", "COEFF2_97.5%_Conf")
