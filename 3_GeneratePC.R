...
Output_PCA <- file.path(Output_root,"PCA")
if (!dir.exists(Output_PCA)) {
	dir.create(Output_PCA)
}

M_CSI <- fetchTable(dbc,"I_MOD_CSI_AC",current_version)
M_CSI <- M_CSI[!is.na(M_CSI[,"MOD_ID"]),]

#model_id <- data.frame()
#for (i in 1:length(modelid_columns)) {
#	if (i == 1) {
#		model_id <- M_CSI[,c("AC_ID","CSI_ID",modelid_columns[i])]
#	} else {
#		model_id <- rbind(model_id,setNames(M_CSI[,c("AC_ID","CSI_ID",modelid_columns[i])], names(model_id)))
#	}
#}
#
#model_id <- model_id[!is.na(model_id$MOD_ID),]
#M_CSI <- unique(model_id)
#MODEL_ID <- na.omit(unique(M_CSI$MOD_ID))
MODEL_ID <- unique(M_CSI$MOD_ID)
...
#	i = 12
	print(i)
	model <- MODEL_ID[i]
	csi <- M_CSI[M_CSI$MOD_ID==model,]$CSI_ID
	ac_id <- M_CSI[M_CSI$MOD_ID==model,]$AC_ID[1]
	...
	PC1_no_series <- ts(PC1_no_series,start=determineTsStartDate(as.yearqtr(index(na.omit(first_diff_ts)))),frequency=determineTsFrequency(as.yearqtr(index(na.omit(first_diff_ts)))))
	
	pc1_ts <- PC1_no_series/sum(PCA_rotation)
	pc1_frame <- data.frame(QUARTER=as.yearqtr(index(na.omit(first_diff_ts))),AC_ID=ac_id,MOD_ID=model,PC1=pc1_ts,row.names=NULL)
	...
	pca_level <- ts(curves_mod%*%PCA_rotation/sum(PCA_rotation),start=determineTsStartDate(as.Date(as.yearqtr(index(curves_mod)))),frequency=determineTsFrequency(as.Date(as.yearqtr(index(curves_mod)))))
	pc1level <- data.frame(QUARTER=as.yearqtr(index(curves_mod)),AC_ID=ac_id,MOD_ID=model,PC1=pca_level,row.names=NULL)
...
table_name <- "O_MOD_CSI"
#try(sqlDrop(dbc, table_name))
dat = merge(M_CSI,CSI_Eigenvectors,by="CSI_ID")
dat = dat[,c("TimeStamp","VERSION","ID","CSI_ID","MOD_ID","AC_ID","EIGENVECTOR","MULTIPLIER")]
#sqlSave(dbc, dat, tablename = table_name, addPK=FALSE, safer = FALSE, rownames=FALSE)
saveTable(dbc,table_name,dat)
